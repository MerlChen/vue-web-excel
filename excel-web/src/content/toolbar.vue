...
export default {
  ...
  methods: {
    ...
    borderSetting(border) {
      this.store.changeBorder(border);
      this.clearStoreInfo();
    },
    /**
     * @description 更改边框颜色
     ...
     */
    mergeCellInfo() {
      let result = this.store.SelectedCells;
      if (this.isMergeCell) {
        this.store.cellMerge(result);
      } else {
        this.store.splitCell(result);
      }
      this.clearStoreInfo();
    },
    /**
    ...
    clearStoreInfo() {
      this.store.startRow = null;
      this.store.endRow = null;
      this.store.startCell = null;
      this.store.endCell = null;
    },
    checkMenuShow(e) {
      let x = e.clientX;
      let y = e.clientY;
